...
type User struct {
	...
	Passcode string    `gorm:"column:passcode;type:char(12);comment:加密随机数;NOT NULL" json:"passcode"`
	Passwd   string    `gorm:"column:passwd;type:char(64);comment:加密后的密文;NOT NULL" json:"passwd"`
	Status   uint      `gorm:"column:status;type:tinyint(4) unsigned;default:0;comment:用户账号状态。0-默认；1-冻结；2-停号;NOT NULL" json:"status"`
...
func (u *User) CheckPassword(password string) (bool, error) {
	err := bcrypt.CompareHashAndPassword([]byte(u.Passcode), []byte(password))
	if err != nil {
		return false, err
	}
	return true, nil
